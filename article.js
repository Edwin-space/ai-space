...
async function init(){const id=new URLSearchParams(location.search).get('id');const data=await fetch('./data/index.json').then(r=>r.json());const story=(data.items||[]).find(x=>x.id===id);if(!story){$('#article').innerHTML='<div class="empty">콘텐츠를 찾을 수 없습니다.</div>';return}
...
if(sources.length)sections.push(`<section class="sources"><h2>Sources</h2>${sources.map(s=>`<a href="${esc(s.url)}" target="_blank" rel="noreferrer">${esc(s.label||s.url)}</a>`).join('')}</section>`);
